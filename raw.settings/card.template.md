...
# `raw.card.*.md` (the `ainp-stale` checker) never picks it up. Rename to `raw.card.<name>.md` to make a real card.
...
- **Two freshness fields, on purpose:** `half_life:` is human prose (readable at a glance); **`half_life_days:` is
  the integer the freshness tooling reads** (`ainp-stale`). Keep BOTH — the researcher writes both on every refresh.
